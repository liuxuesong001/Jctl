...
public class ToastUtils {
    ...
    private static Toast createToast(CharSequence msg, int duration) {
        if (toast_1 == null) {
            toast_1 = Toast.makeText(JctlApplication.getContext(), msg, duration);
        } else {
            toast_1.setText(msg);
    ...
    public static void showShort(int strId) {
        createToast(JctlApplication.getContext().getText(strId), Toast.LENGTH_SHORT).show();
    }
    ...
    public static Toast showToastWithImg(final String tvStr, final int imageResource) {
        if (toast_2 == null) {
            toast_2 = new Toast(JctlApplication.getContext());
        }
        View view = LayoutInflater.from(JctlApplication.getContext()).inflate(R.layout.toast_custom, null);
